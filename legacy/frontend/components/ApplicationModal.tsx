...
import { APPLY_SLIDES } from '../lib/heroSlides';
  ...
  return (
    <section className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
      <div className="flex items-center gap-3 mb-5">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/20 text-yas-midnight">
          <Icon size={20} aria-hidden="true" />
        </div>
        <h2 className="text-lg font-bold text-yas-midnight">
          {title}
        </h2>
      </div>
      {children}
...
export default function ApplicationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onClose();
      toast.success('Candidature envoyée — un e-mail de confirmation vous a été adressé');
    } catch (error: any) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4">
      ...
      <div className="relative flex h-[min(92vh,820px)] w-full max-w-6xl overflow-hidden rounded-2xl bg-white shadow-xl">
        {/* Colonne images — desktop */}
        <div className="relative hidden h-full w-[42%] shrink-0 lg:block">
          <HeroCarousel
            slides={APPLY_SLIDES}
            sizes="42vw"
            className="absolute inset-0"
            overlayClassName="absolute inset-0 bg-[#00377D]/50"
            showDots
          />
          <div className="absolute inset-x-0 bottom-0 z-[4] p-8 text-white">
            <img src="/jm.svg" alt="YAS" className="mb-4 h-12 w-auto" />
            <p className="text-lg font-semibold leading-snug">
              Postule chez YAS Togo et rejoins une équipe engagée.
            </p>
          </div>
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className="sticky top-0 z-10 flex shrink-0 items-center justify-between border-b border-gray-200 bg-white px-5 py-4 sm:px-6">
            <div>
              <h1 className="text-xl font-bold sm:text-2xl text-yas-midnight">
                Candidature au poste
              </h1>
              <div
                className="mt-2 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-semibold bg-accent text-yas-midnight"
              >
                <Briefcase size={14} />
                {jobTitle}
              </div>
          ...
          {/* Carousel mobile */}
          <div className="relative h-36 w-full shrink-0 lg:hidden">
            <HeroCarousel
              slides={APPLY_SLIDES}
              sizes="100vw"
              className="absolute inset-0"
              overlayClassName="absolute inset-0 bg-[#00377D]/45"
            />
          </div>

          <form onSubmit={handleSubmit} className="min-h-0 flex-1 space-y-6 overflow-y-auto p-5 sm:p-6">
                ...
                <div className="flex items-center gap-8">
                  <label className="flex items-center gap-2 text-sm text-gray-700">
                    <input
                      ...
                      className="accent-yas-midnight h-4 w-4"
                    />
                    Masculin
                  </label>
                  <label className="flex items-center gap-2 text-sm text-gray-700">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-t border-gray-200 pt-6">
            ...
            <button
              ...
              className="rounded-xl px-6 py-3 font-bold text-[#00377D] shadow-sm transition-opacity hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 bg-primary text-primary-foreground font-bold"
            >
              {isSubmitting ? (
                <>
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-[#00377D] border-t-transparent" />
                  Envoi en cours...
                </>
              ) : (

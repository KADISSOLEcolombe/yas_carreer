...
import { CircleCheck, CircleX, Filter, Download, Search, X, Clock, Eye } from 'lucide-react';
...
function ScoreBadge({ score, detail }: { score?: number | null; detail?: string }) {
  if (score === null || score === undefined) return null;
  const style = score >= 70 ? { bg: '#D1FAE5', text: '#065F46' } : score >= 40 ? { bg: '#FEF3C7', text: '#92400E' } : { bg: '#FEE2E2', text: '#DC2626' };
  return (
    <span
      title={detail || 'Score de compatibilité avec l’offre'}
      className="inline-flex rounded-full px-3 py-1 text-xs font-semibold cursor-help"
...
export default function RHCandidaturesPage() {
  ...
  const [sortBy, setSortBy] = useState<'date' | 'score_desc' | 'score_asc'>('score_desc');
  ...
  const activeFiltersCount = (statusFilter !== 'TOUS' ? 1 : 0) + (searchTerm ? 1 : 0) + (minScore !== '' ? 1 : 0);
          ...
          <div className="border-b border-gray-100 px-6 py-4 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                ...
                className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm"
              >
                <option value="score_desc">Compatibilité ↓</option>
                <option value="score_asc">Compatibilité ↑</option>
        ...
        <div className="px-6">
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              {activeFiltersCount > 0 && (
                <button
                  onClick={() => { setSearchTerm(''); setStatusFilter('TOUS'); setMinScore(''); }}
            ...
            filteredCandidatures.map((c) => (
              ...
              >
                <Link href={`/rh/candidatures/${c.id}`} className="flex min-w-0 flex-1 items-center gap-3">
                  <div
                  ...
                  <div className="min-w-0">
                    <p className="truncate font-bold text-gray-900">{c.prenom ? `${c.prenom} ${c.nom}` : c.nom}</p>
                    <p className="truncate text-sm text-gray-500">
                      {c.jobTitle} · {new Date(c.createdAt).toLocaleDateString('fr-FR')}
                    </p>
                    {c.matching?.detail && (
                      <p className="truncate text-xs text-gray-400 mt-0.5">{c.matching.detail}</p>
                    )}
                  </div>
                </Link>

                <div className="flex shrink-0 items-center gap-2">
                  <ScoreBadge score={c.score} detail={c.matching?.detail} />
                  <StatusBadge status={c.status} />
                  <Link
                    href={`/rh/candidatures/${c.id}`}
                    className="flex h-8 w-8 items-center justify-center rounded-lg text-[#00377D] transition-colors hover:bg-[#FFD100]/30"
                    title="Voir les détails"
                  >
                    <Eye size={16} />
                  </Link>
                  <button
                    onClick={() => handleChangeStatus(c.id, 'PENDING')}
                  ...
                  <button
                    onClick={() => handleChangeStatus(c.id, 'IN_REVIEW')}
                  ...
                  <button
                    onClick={() => handleChangeStatus(c.id, 'ACCEPTED')}
                  ...
                  <button
                    onClick={() => handleChangeStatus(c.id, 'REJECTED')}

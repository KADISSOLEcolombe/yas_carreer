...
import { ArrowLeft, User, Briefcase, Calendar, MapPin, Phone, Mail, FileText, Download, Check, X, UserCog, Clock, Sparkles, RefreshCw } from 'lucide-react';
import { api, mapCandidature, mapStatusToBackend, authenticatedFileUrl, type Application } from '../../../../lib/api';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl font-bold mb-2" style={{ color: COLORS.midnight }}>
                {application.prenom ? `${application.prenom} ${application.nom}` : application.nom}
              </h1>
        ...
        <SectionCard icon={Sparkles} title="Analyse de correspondance">
          <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm text-gray-600">
                {application.matching?.detail ||
                  (application.score != null
                    ? `Score enregistré : ${application.score}%`
                    : "Score indisponible — renseignez les compétences de l'offre et du candidat.")}
              </p>
              <button
                type="button"
                onClick={async () => {
                  try {
                    const updated = await api.recalculerScore(Number(id));
                    setApplication(mapCandidature(updated));
                  } catch (err: any) {
                    alert(err.message || 'Erreur lors du recalcul');
                  }
                }}
                className="inline-flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-1.5 text-sm font-medium text-[#00377D] hover:bg-[#FFD100]/20"
              >
                <RefreshCw size={14} />
                Recalculer
              </button>
            ...
            {application.matching && (
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Compétences en commun</p>
                  <div className="flex flex-wrap gap-2">
                    {(application.matching.competencesMatch || []).length === 0 ? (
                      <span className="text-sm text-gray-400">Aucune</span>
                    ) : (
                      application.matching.competencesMatch.map((c) => (
                        <span key={c} className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700">
                          {c}
                        </span>
                      ))
                    )}
                ...
                <div>
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Compétences manquantes</p>
                  <div className="flex flex-wrap gap-2">
                    {(application.matching.competencesManquantes || []).length === 0 ? (
                      <span className="text-sm text-gray-400">Aucune</span>
                    ) : (
                      application.matching.competencesManquantes.map((c) => (
                        <span key={c} className="rounded-full bg-red-50 px-2.5 py-1 text-xs font-medium text-red-700">
                          {c}
                        </span>
                      ))
                    )}
            ...
            {application.matching && (
              <p className="text-sm text-gray-600">
                Localisation :{' '}
                <span className={application.matching.localisationOk ? 'font-semibold text-emerald-700' : 'font-semibold text-amber-700'}>
                  {application.matching.localisationOk ? 'correspondante' : 'non correspondante'}
                </span>
              </p>
            )}
          </div>
            ...
            {files.length === 0 ? (
              ...
              files.map((file) => (
                <div key={file.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <a
                    href={authenticatedFileUrl(file.id)}
                    target="_blank"
            ...
            {emploi ? (
              ...
              <div>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold text-[#00377D] transition-opacity hover:opacity-90"
                  style={{ backgroundColor: COLORS.yellow }}
                >
                  <UserCog size={18} />
                  Affecter à un superviseur
                </button>
              </div>
            )}
          </SectionCard>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900">Affecter un employé / stagiaire</h2>
              <button

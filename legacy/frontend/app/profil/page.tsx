...
import { api, mapOffre, authenticatedFileUrl, type Job } from '../../lib/api';
...
function computeProfilCompletion(user: {
  nom?: string | null;
  prenom?: string | null;
  email?: string | null;
  ...
  if (!user) return 0;
  const fields = [user.nom, user.prenom, user.email, user.telephone, user.quartier, user.competences, user.ville];
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        try {
          const entretiensData = await api.getMesEntretiensCandidat();
                            ...
                            {mesDocuments.map((doc) => (
                              <div key={doc.id} className="flex items-center justify-between rounded-xl border border-gray-100 p-3">
                                ...
                                <div className="flex shrink-0 items-center gap-1">
                                  <a
                                    href={authenticatedFileUrl(doc.id)}
                                    target="_blank"
                      ...
                      <div>
                        ...
                        <div className="h-2 w-full rounded-full bg-gray-100">
                          <div
                            ...
                            style={{
                              width: `${computeProfilCompletion(user)}%`,

...
/** URL de téléchargement authentifiée (?token=) pour liens <a href>. */
export function authenticatedFileUrl(cheminOrId: string | number): string {
  const token = typeof window !== 'undefined' ? localStorage.getItem('yas_token') : null;
  let path: string;
  if (typeof cheminOrId === 'number') {
    path = `/api/files/${cheminOrId}/content`;
  } else if (cheminOrId.startsWith('/api/files/')) {
    path = cheminOrId;
  } else if (cheminOrId.startsWith('/uploads/')) {
    // Ancien format — non supporté sans id ; renvoyer tel quel (échouera côté API)
    path = cheminOrId;
  } else {
    path = cheminOrId.startsWith('/') ? cheminOrId : `/${cheminOrId}`;
  }
...
export interface MatchingAnalyse {
  score: number | null;
...
export function mapCandidature(apiCandidature: ApiCandidature): Application {
  return {
    ...
    prenom: apiCandidature.utilisateur?.prenom || '',
    email: apiCandidature.utilisateur?.email || '',
    telephone: apiCandidature.utilisateur?.telephone || '',
    coverLetter: '',
    cvFileName: '',
    score: apiCandidature.score ?? apiCandidature.matching?.score ?? null,
    matching: apiCandidature.matching,
  };
}
...
export const api = {
  ...
  getAllApplications: (params?: { statut?: string; minScore?: number; id_offre?: number; sort?: string }) => {
    const q = new URLSearchParams();
    if (params?.statut) q.set('statut', params.statut);
    if (params?.minScore != null) q.set('minScore', String(params.minScore));
    if (params?.id_offre != null) q.set('id_offre', String(params.id_offre));
    if (params?.sort) q.set('sort', params.sort);
    const qs = q.toString();
    return request<ApiCandidature[]>(`/api/candidatures${qs ? `?${qs}` : ''}`);
  },

  recalculerScore: (id: number) =>
    request<ApiCandidature>(`/api/candidatures/${id}/recalculer-score`, { method: 'POST' }),
  ...
  getMesEntretiensCandidat: () =>
    request<any[]>('/api/entretiens/candidat', {
      method: 'GET',
    }),
  ...
  getMesEmplois: () =>
    request<any[]>('/api/emplois/mes'),

  getEmploiById: (id: number) =>
    request<any>(`/api/emplois/${id}`),

  updateEmploi: (id: number, data: {
    id_departement?: number;
    date_debut?: string;
    date_fin?: string;
    sujet?: string;
    lieu?: string;
    utilisateursup_id?: number | null;
    statut?: string;
  }) =>
    request<any>(`/api/emplois/${id}`, {
      method: 'PUT',

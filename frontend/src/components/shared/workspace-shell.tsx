...
export function WorkspaceShell({
  ...
  workspaceLabel,
  primaryCta,
  tagline = "Voici l'état du recrutement Yas Togo aujourd'hui.",
}: {
  ...
  const notificationsHref = homeHref.startsWith("/admin")
    ...
    : homeHref.startsWith("/rh")
      ? "/rh/notifications"
      : homeHref.startsWith("/superviseur")
        ? "/superviseur/notifications"
        : homeHref.startsWith("/candidat")
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          active
            ? "bg-yas-yellow text-yas-midnight shadow-sm"
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] text-slate-800">
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        {/* Sidebar — bleu marine dominant, actifs en jaune */}
        <aside className="sticky top-0 hidden h-screen w-[260px] shrink-0 flex-col bg-yas-midnight px-4 py-5 lg:flex">
          <div className="mb-5 px-1">
            <BrandLogo href={homeHref} size="md" variant="light" />
            <p className="mt-1 truncate pl-[3.25rem] text-[11px] text-white/50">
          ...
          <div className="relative mb-5">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-white/10 bg-white/10 pl-9 pr-12 text-sm text-white outline-none transition placeholder:text-white/40 focus:border-yas-yellow/60 focus:bg-white/15 focus:ring-2 focus:ring-yas-yellow/20"
            />
            <kbd className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 rounded-md border border-white/10 bg-white/10 px-1.5 py-0.5 text-[10px] font-medium text-white/40">
              ⌘K
            </kbd>
          </div>
          ...
          {primaryCta && (
            <Link
              ...
              className="mb-5 flex items-center justify-center gap-2 rounded-xl bg-yas-yellow px-3 py-2.5 text-sm font-semibold text-yas-midnight shadow-md shadow-black/10 transition hover:bg-yas-yellow/90"
            >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className="mt-4 flex w-full items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-2.5 py-2.5 text-left transition hover:bg-white/10"
              >
                <Avatar className="size-9 bg-yas-yellow">
                  <AvatarFallback className="bg-yas-yellow text-xs font-semibold text-yas-midnight">

...
async function searchDuckDuckGoHtml(query: string): Promise<WebSource[]> {
  const res = await fetch('https://html.duckduckgo.com/html/', {
    ...
    headers: {
      ...
      'User-Agent':
        'Mozilla/5.0 (compatible; YasCareerBot/1.0; +https://yascareer.tg)',
    },
...
export default class WebResearchService {
  static isConfigured() {
    return Boolean(env.get('TAVILY_API_KEY') || env.get('SERPER_API_KEY'))
  }

  static async researchPerson(input: {
    fullName?: string | null
    email?: string | null
    skills?: string | null
    offerTitle?: string | null
    ...
    const tavily = env.get('TAVILY_API_KEY')
    const serper = env.get('SERPER_API_KEY')
    const all: WebSource[] = []
    ...
    for (const query of queries) {
      try {
        ...
        if (tavily) {
          ...
          if (!batch.length) {
            batch = await searchDuckDuckGoHtml(query)
          }
          provider = batch.length ? 'duckduckgo' : provider
        }
        all.push(...batch)
      } catch (error) {
        logger.warn({ err: error, query }, 'Web research query failed')
      }

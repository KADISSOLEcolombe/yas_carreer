...
type SeedOffer = {
  ...
  location: string
  deadlineDays: number
  status: 'publiee' | 'brouillon' | 'fermee'
  requirements: string
  description: string
}
...
export default class extends BaseSeeder {
  async run() {
    // Reset métier data for a clean demo dataset
    await Interview.query().delete()
    await ApplicationStatusHistory.query().delete()
    await Application.query().delete()
    await Notification.query().delete()
    await ChatbotMessage.query().delete()
    ...
    const rh = await User.updateOrCreate(
    ...
    for (const c of CANDIDATES) {
      ...
      await CandidateProfile.updateOrCreate(
        { userId: user.id },
        {
          bio: c.bio,
          skills: c.skills,
    ...
    let published = 0
    const createdOffers: Awaited<ReturnType<typeof Offer.create>>[] = []
    for (const item of OFFERS) {
      const offer = await Offer.create({
        ...
        deadline: DateTime.now().plus({ days: item.deadlineDays }),
    ...
    // Candidatures démo pour tester le classement IA
    const candidats = await User.query().where('role', 'candidat')
    ...
    for (const [idx, offer] of demoTargets.entries()) {
      for (const [cIdx, candidat] of candidats.entries()) {
        ...
        await Application.create({
          ...
          appliedAt: DateTime.now().minus({ days: cIdx + idx }),
        })
        appsCreated++
      }
    }
    ...
    console.log('Seed Yas Togo OK', {
      ...
      candidats: CANDIDATES.map((c) => c.email),
